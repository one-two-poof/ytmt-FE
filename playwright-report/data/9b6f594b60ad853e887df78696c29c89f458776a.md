...
- Name: components.test.ts >> Input >> 포커스 상태
- Location: tests/visual/components.test.ts:88:7

# Error details

```
Error: locator.focus: Error: strict mode violation: getByTestId('section-input').getByPlaceholder('오늘 어떤 생각을 하셨나요?') resolved to 2 elements:
    1) <input class="Input_base__19n8zsq0" placeholder="오늘 어떤 생각을 하셨나요?"/> aka getByRole('textbox', { name: '오늘 어떤 생각을 하셨나요?', exact: true })
    2) <textarea rows="4" class="Input_base__19n8zsq0" placeholder="오늘 어떤 생각을 하셨나요? 솔직하게 적어보세요."></textarea> aka getByRole('textbox', { name: '오늘 어떤 생각을 하셨나요? 솔직하게 적어보세요' })

Call log:
  - waiting for getByTestId('section-input').getByPlaceholder('오늘 어떤 생각을 하셨나요?')
